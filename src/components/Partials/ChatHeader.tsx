...
const ChatHeader: React.FC<Props> = ({ ...props }) => {
  ...
  const { getMessageDefaultChannel, searchAddress,alert,verifyContractAndRemove,loading,account, isOwner, get_private } = useContract();
  ...
  return (
    <div className="chat-header">
      <div className="chat-header-user">
        ...
        <div>
          <h5>{name} {isPrivate ? <i className="ti ti-lock"></i>: <i className="ti ti-world"></i>} </h5>
          <small className="text-muted" onClick={()=>copy()}>
            <i> {truncateText(props?.selectedChat, 7, 10, false)}</i>
          </small>     
          <p className="text-muted">{isOwnerAddUser ? t("TEXT_OWNER") : t("TEXT_MEMBER")}</p>     
        </div>
      </div>
      ...
      <div className="chat-header-action">
        <ul className="list-inline">
          ...
          data-toggle="tooltip"
          title="Add User">
            <IonButton onClick={()=>setModalAddUser(true)} size="small" color={"warning"}>
          ...
          <li
            ...
            title="Unsubscribe"
          >
            <IonButton color="danger" onClick={()=>removeChannel()} size="small">
              <i className="ti ti-trash"></i>
            </IonButton>
          </li>
        </ul>
      </div>
      <IonLoading trigger="open-loading" isOpen={loading} message="Loading..." duration={3000}  spinner="circles" />
      <ShareChannelModal address={props.selectedChat} modal={modal} modalToggle={modalToggle} name={name} />
